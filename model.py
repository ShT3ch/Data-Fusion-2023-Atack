...
# python model.py ../data/transactions.csv result.csv
...
class TransactionsRnn(pl.LightningModule):
    ...
    def forward(self, transactions_cat_features, *, return_embedding=False):
        ...
        concated_embeddings = self._get_input_embs(transactions_cat_features)
        last_hidden, states = self.get_emb(concated_embeddings)
    ...
    def get_seq_representation(self, transactions_cat_features):
        batch_size = transactions_cat_features.shape[0]
        
        concated_embeddings = self._get_input_embs(transactions_cat_features)
        last_hidden, states = self.get_emb(concated_embeddings)
        return self.assemble_seq_encoding(batch_size, last_hidden, states)
    
    def assemble_seq_encoding(self, batch_size, last_hidden, states):
    ...
    def classify_emb(self, batch_size, last_hidden, states):
        combined = self.assemble_seq_encoding(batch_size, last_hidden, states)
        drop = nn.functional.dropout(combined, p=0.5)
        logit = self._top_classifier(drop)
        probs = nn.functional.softmax(logit, dim=1)
        return probs
    
    def _get_input_embs(self, transactions_cat_features):
        embeddings = [
    ...
    def get_emb(self, concated_embeddings):
    ...
    def training_step(self, train_batch, batch_idx):
        self.train()
        idx, x, y_label, transaction_source_id, _, is_pseudo_label = train_batch
        y_label = y_label.float()
        batch_size = len(y_label)
        y_pred = self.forward(x)
        
        mask2relable = [((i.item() in self.pseudo_labels_store) and (is_pl.item() == True)) for i, is_pl in zip(idx, is_pseudo_label)]
        relabeling_change = [self.pseudo_labels_store[i.item()] for i, is_pl in zip(idx, is_pseudo_label) if i.item() in self.pseudo_labels_store and is_pl.item() == True]
        
        assert sum(mask2relable) == len(relabeling_change), (mask2relable, relabeling_change)
        
        if len(relabeling_change) > 0:
            mask2relable = torch.tensor(mask2relable, device=self.device, dtype=torch.bool)
            relabeling_change = torch.tensor(relabeling_change, device=self.device, dtype=torch.float)
            
            y_label[mask2relable] = relabeling_change
        
        for i, is_pl, pred_level in zip(idx, is_pseudo_label, y_pred[:, 1]):
            thresh = 0.06
            if is_pl.item() == True:
                if i.item() in self.pseudo_labels_store:
                    self.pseudo_labels_store[i.item()] = 0.8 if  pred_level.item() > thresh else 0.01
#                     self.pseudo_labels_store[i.item()] = (self.pseudo_labels_store[i.item()] + pred_level.item())/2
                else:
                    self.pseudo_labels_store[i.item()] = 0.8 if pred_level.item() > thresh else 0.01
#                     self.pseudo_labels_store[i.item()] = pred_level.item()
            
        
        batch_loss = self.loss(y_pred[:, 1], y_label, reduction='none')
        loss_weight = torch.ones_like(batch_loss, requires_grad=False)
        loss_weight[is_pseudo_label] = 0.05*self.current_epoch
        batch_loss = (batch_loss*loss_weight).mean()

        y_pred_cpu = y_pred.detach().cpu().numpy()[:, 1]
        
        try:
            rocauc = roc_auc_score(y_label.detach().cpu().numpy()>0.5, y_pred_cpu)
        except:
            rocauc = 0
        accuracy = accuracy_score(y_label.detach().cpu().numpy()>0.5, y_pred_cpu > 0.15)
    ...
    def validation_step(self, val_batch, batch_idx):
        x, y_label, transaction_source_id, _ = val_batch
        batch_size = len(y_label)
        y_pred = self.forward(x)
...
class TransactionsDataset(Dataset):
    def __init__(self, data, dftarget=None, is_pseudo_labeled=None):
        self.is_pseudo_labeled = is_pseudo_labeled
        if dftarget is not None:
            d = data.merge(dftarget, on="user_id")
            self.labels = d.target.copy().values
            self.features = d.sequences.apply(lambda x: x[:-2]).copy().values
            self.is_pos_amt = d.sequences.apply(lambda x: x[-2]).copy().values
            self.source_transaction_id = d.sequences.apply(lambda x: x[-1]).copy().values
            del d
        else:
            self.labels = data.user_id.copy().values
            self.features = data.sequences.apply(lambda x: x[:-2]).copy().values
            self.is_pos_amt = data.sequences.apply(lambda x: x[-2]).copy().values
            self.source_transaction_id = data.sequences.apply(lambda x: x[-1]).copy().values

    def __len__(self):
        return len(self.features)

    def __getitem__(self, idx):
        x, y, z, zz = self.features[idx], self.labels[idx], self.source_transaction_id[idx], self.is_pos_amt[idx]
        if self.is_pseudo_labeled is not None:
            return idx, x, y, z, zz, self.is_pseudo_labeled
        else:
            return x, y, z, zz
...
def get_dataloader(dataset, device, batch_size=128, is_validation=False, nworkers=0):
    def collate_loader(x):
        return tuple(x_.to(device) for x_ in default_collate(x))

    return DataLoader(dataset, batch_size=batch_size, collate_fn=collate_loader, shuffle=not is_validation, num_workers=nworkers)
...
def predict(source_file, bins_path, model_path, random_seed=20230206):
    ...
    preds = []
    users = []
    for data, target, _, __ in dataloader:
        y_pred = model(data)
